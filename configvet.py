...
# ----------------------------------------------------------------------
# 14. LINUX AUDITD (BASIC STIG) RULES PARSER & ANALYZER
# ----------------------------------------------------------------------
class AuditdAuditor:
    """Audit Linux audit.rules against basic STIG requirements."""
    def __init__(self, content):
        self.content = content
    ...
    def audit(self):
        required = [
            {"pattern": r"-w /etc/group", "msg": "STIG: /etc/group writes should be monitored"},
            {"pattern": r"-w /etc/passwd", "msg": "STIG: /etc/passwd writes should be monitored"},
            {"pattern": r"-w /etc/shadow", "msg": "STIG: /etc/shadow writes should be monitored"},
            {"pattern": r"-w /etc/sudoers", "msg": "STIG: /etc/sudoers writes should be monitored"},
            {"pattern": r"-a always,exit -S adjtimex", "msg": "STIG: adjtimex syscall should be audited"},
            {"pattern": r"-a always,exit -S settimeofday", "msg": "STIG: settimeofday syscall should be audited"},
            {"pattern": r"-a always,exit -S execve.*-F.*uid>=1000", "msg": "STIG: execve for users should be audited"},
            {"pattern": r"-a always,exit -S init_module", "msg": "STIG: module loading should be audited"},
            {"pattern": r"-a always,exit -S delete_module", "msg": "STIG: module unloading should be audited"},
        ]
        rule_text = "\n".join(self.rules)
        for req in required:
...
def main():
    ...
    parser.add_argument("--auditd", help="Linux audit.rules file (basic STIG checks)")
